...
setwd("clustermanycov.simulation/Table 1-4")
...
for (m in models){
  ...
  for (s in 1:S) {
    ...
    for (K.i in K.grid) {
      ...
      if ( m == 2 | m == 4) {dgp = gen.data(q=q,qx=qx,n=n,n_prime=n_prime,nx=nx,Jx=Jx,Ju=Ju,rho=rho,l=l,m,beta=beta ,K.i=K.i, K=K.grid.LM[K.i]); W = dgp$W}
      
      K = ncol(W); qr.W = qr(W); if (qr.W$rank==K){
        M = -tcrossprod(qr.Q(qr.W)); diag(M) = 1 + diag(M);
        Mn = 1-min(diag(M))
        if (m == 1| m == 3) {ratio = Mn*((n+n_prime)/2)^{2}}
        if (m == 2| m == 4) {ratio = Mn*(max(n,n_prime))^{2}}
  ...
  ## Save final table
  filename = paste0("output2/output_m",m,".csv");
